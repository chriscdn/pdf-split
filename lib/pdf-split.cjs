...
const quote = text => shellQuote.quote([text]);
  ...
  constructor(args) {
    ...
    const cb = async (filePath, {
      ...
      if (rotateResolved === exports.Rotate.DEG_0) {
        ...
        const command = [this.pdfcpu, "rotate", quote(sourceFilePath), rotateResolved, quote(filePath)];
        await execPromise(command.join(" "));
      }
    };
    super({
      ...
      resolveCacheKey: ({
        ...
        pageIndex,
        rotate
      }) => sha1__default["default"](JSON.stringify({
        pdfFilePath,
        pageIndex,
