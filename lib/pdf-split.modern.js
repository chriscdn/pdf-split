...
import { quote as quote$1 } from 'shell-quote';
...
const isPasswordRequiredException = e => {
  var _e$stderr;
  return isExecException(e) && ((_e$stderr = e.stderr) == null ? void 0 : _e$stderr.trim()) === "pdfcpu: please provide the correct password";
};
...
const optionsToCLI = options => [...(options.userPassword ? ["-upw", quote(options.userPassword)] : []), ...(options.ownerPassword ? ["-opw", quote(options.ownerPassword)] : [])];
const randomDirectoryName = (l = 16) => [...Array(l)].map(() => Math.random().toString(36)[2]).join("");
const quote = text => quote$1([text]);
class PDFSplitFileCache extends FileCache {
  constructor(args) {
    var _this, _args$pdfcpu;
    // Make a standalone function that can be recursively called.
    const cb = async function cb(filePath, {
      pdfFilePath,
      pageIndex,
      rotate
    }) {
      ...
      if (rotateResolved === Rotate.DEG_0) {
        ...
        const sourceFilePath = await _this.resolveFilePath({
          pdfFilePath,
          pageIndex
        });
        ...
        const command = [_this.pdfcpu, "rotate", quote(sourceFilePath), rotateResolved, quote(filePath)];
        await execPromise(command.join(" "));
      }
    };
    super({
  ...
  async pdfInfo(pdfFilePath, options = {}) {
    const command = [this.pdfcpu, "info -json", ...optionsToCLI(options), quote(pdfFilePath)];
    // console.log("**************");
    // console.log(command);
    // console.log("**************");
    const {
