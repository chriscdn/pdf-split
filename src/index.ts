...
class PDFSplitFileCache extends FileCache<PDFArgs> {
  ...
  async pageCount(pdfFilePath: FilePath): Promise<PDFCpuPageInfo["pageCount"]> {
    const { pageCount } = await this.pdfInfo(pdfFilePath);
    return pageCount;
  }

  async pages(pdfFilePath: FilePath): Promise<FilePath[]> {
    const pageCount = await this.pageCount(pdfFilePath);

    return await Promise.all(
      Array.from({ length: pageCount }, (_, i) =>
        this.getFile({ pdfFilePath, pageIndex: i }),
      ),
    );
  }
}
